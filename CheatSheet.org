...
* COMMENT The type of types is ‘type’, which is its own type!
#+begin_src python
# The type of types is ‘type’, which is its own type!
assert type(1)    == int
...
# ⇒ Python supports simultaneous assignment; e.g., ~x , y = y , x~ swaps ~x~ and ~y~.
...
two ways: ~obj.f()~ or ~τ.f(obj)~. The latter hints at why src_python[:exports
code]{“self”} is the usual name of the first argument to instance methods. The
former ~τ.f~ is the name proper.

⇒ Function and class definitions are the only way to introduce new, local,
  scope.
  # In-particular, “with” and “:=” introduce new names.
...
#+end_parallel
#
# -  =x and y ≈ x= for any falsey =x=; i.e., if ~bool(x)~ is false.
...
# Perform =sₙ= only if all =sᵢ= are not =None= (falsey); e.g.,
| =s₁ and ⋯ and sₙ= | ⇒ | Do ~sₙ~ only if all ~sᵢ~ “succeed” |
| ~s₁ or ⋯ or sₙ~   | ⇒ | Do ~sₙ~ only if all ~sᵢ~ “fail”    |
# /are/ ~None~ (falsey) e.g., do =sₙ= if all ~sᵢ~ “fail”.
...
** COMMENT Further equivalences :Iverson:
 Provided no bᵢ is falsy:

   c₀ and b₀ or ⋯ or cₙ and bₙ
 ≈ b₀ if c₀ else ⋯ else bₙ if cₙ

 Since tuples can be indexed:
 (a,b)[c] ≈ a if c else b  (Strict!)

 If neither a nor b are falsy:
 a * bool(c) or b ≈ a if c else b
...
:End:
 ...
 C#.
:More:
  # do x ← a; y ← b; return c ≈ (c for x in a for y in b)
  # #
...
we may write @@latex:\newline@@ ~x , y = e₁, e₂~ thereby obtaining *simultaneous assignment*.
@@latex:\newline@@
...
+ src_python[:exports code]{len} gives the length of (finite) iterables
  ...
  :Examples:
   #+BEGIN_SRC python
assert 2 == len ('hi')
assert 2 == len ((1, 2)) # ⇒ Extra parens to make it clear we're giving ONE tuple argument, not TWO integer arguments.
assert 2 == len ([1, 2])
assert 2 == len ({'h', 'i'})
...
#+end_src
   ...
   E.g., ~list(zip(*[(x₁, y₁), (x₂, y₂)])) ≈ list(zip((x₁, y₁), (x₂, y₂))) ≈ [(x₁,
   x₂), (y₁, y₂)]~, hence we “unzip” using =zip(*⋯)=.  ⇒ Note we need ~list(⋯)~ to
   actually get a list since =zip= just returns an abstract iterator of tuples. ⇐
    :End:
   #+begin_src python
xs , τ = [ {1,2} , [3, 4] ] , list
...
@@latex:\newline@@ ~[x₀, …, xₙ] < [y₀, …, yₘ] ≡ x₀ < y₀ or ⋯ or xₖ < yₖ~
...
:Example:
#+begin_src python
"!".join(['a', 'bob', 'jim']) # ⇒ a!bob!jim
'a!bob!jim'.split('!') # ⇒ ['a', 'bob', 'jim']
...
:StringSpecfifiers:
# Strings are formatted using “specfication-string % arguments”:
...
| %.𝓃f | floats with 𝓃 decimal values |
:End:
...
# ( Classes are dictionaries with nifty ways to “mix in” functionality; e.g.,
...
# + The ~[::-1]~, for reversal, above is known as the “alien smiley face”.
# Maybe not.
 ...
 them anywhere, such as storing them in lists.
  ...
  + Any object ~x~ can be treated like a function, and use the ~x(⋯)~ application
    ...
    callable or not.
     ...
     # If ~callable(x)~ then ~x(⋯)~ is valid syntax;
     # ~callable(x) ≈ assert hasattr(x, '__call__')~
...
+ E.g., if =f= needs 3 arguments, then =f(*[x₁, x₂, x₃]) ≈ f(x₁, x₂, x₃)=.
 ...
 #+begin_src python
args = [0, 5]
print (list(range(*args))) # ⇒ [0, 1, 2, 3, 4]
   #+end_src
:End:
...
#
#+begin_parallel org
# To partially apply a function ~f~ with values ~vᵢ~, use ~partial(f, v₀, …, vₙ)~.
#+latex: \vspace{0em}
#+begin_src python
   partial(f, v₀, …, vₙ)
≈ lambda x₀, …, xₘ:
...
#+begin_src python
def doit(howmany):
    # Repeat a function “howmany” times
...
doit('a', z = 2, y = {})
  ...
  #+end_src
# print(help(doit))
# ⇒ It would say “typed.new”, from above!
...
Objects are like dictionaries, but classes allow us to “mix in” functionality
...
#+begin_center python
src_python[:exports code]{class X: a = 1  ≡  X = type('X', (object,), dict(a = 1))}
#+end_center
...
#+end_src
 ...
 operators]] ~__rℴ𝓅__~ are used for arguments of different types: @@latex:
 \newline@@ ~x ⊕ y ≈ y.__r⊕__(x)~ if ~x.__⊕__(y)~ is not implemented.
...
# a = A() # ⇒ “A” has an abstract method, so cannot make values of this type.
b = B()
c = C()
print (b.doit(), c.doit())
#+end_src
...
#+latex: \vspace{-0.5em}
# which is ~lambda xₖ₊₁: …: lambda xₙ: f(v₁, …, vₖ, xₖ₊₁, …, xₙ)~.
...
# arguments.
#
# ⇒ Currying is not a big issue, since we have access to * and default
# arguments. ⇐
#
# +latex: \newpage
...
⇒ If ~P~ is a package and ~M~ is a module in it, then we can use src_python[:exports
